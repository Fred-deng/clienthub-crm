...
export default function Receivables() {
  ...
  const totals = filtered.reduce(
    ...
    { contract: 0, invoiced: 0, received: 0, outstanding: 0 },
  );
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <KpiCard label="已回款" value={fmtMoney(totals.received)} tone="accent" icon={<ArrowDownLeft className="h-4 w-4" />} />
        <KpiCard label="未收账款" value={fmtMoney(totals.outstanding)} tone="warning" icon={<AlertCircle className="h-4 w-4" />} />
      </div>
              ...
              {filtered.map((r) => {
                ...
                return (
                  <tr key={r.customerId}>
                    ...
                    <td className="num text-mint">{fmtMoney(r.received)}</td>
