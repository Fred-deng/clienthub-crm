...
export function MSearchBar({ value, onChange, placeholder = "搜索…", trailing, hint }: { value: string; onChange: (v: string) => void; placeholder?: string; trailing?: ReactNode; hint?: string }) {
  return (
    <div className="px-4 pt-1 pb-2">
      {hint && <div className="mb-1.5 pl-1 text-[11px] text-foreground/50">搜索范围：{hint}</div>}
      <div className="flex items-center gap-2">
        <div className="flex-1 flex items-center gap-2 h-10 rounded-full bg-card px-3.5 border border-foreground/15 shadow-sm">
          <Search className="h-4 w-4 text-foreground/55" />
          <input
            value={value}
            onChange={(e) => onChange(e.target.value)}
            placeholder={placeholder}
            className="flex-1 min-w-0 bg-transparent outline-none text-sm placeholder:text-foreground/45"
          />
          {value && (
            <button onClick={() => onChange("")} className="size-5 rounded-full bg-foreground/20 flex items-center justify-center">
