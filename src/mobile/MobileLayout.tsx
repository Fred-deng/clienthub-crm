...
import {
  ...
  ArrowDownToLine, ArrowUpFromLine, Scale, X, ChevronRight, Bell,
} from "lucide-react";
import { useCurrentUser } from "@/context/CurrentUserContext";
import { cn } from "@/lib/utils";

const tabs = [
  { to: "/m", label: "首页", icon: LayoutDashboard, end: true },
  { to: "/m/customers", label: "客户", icon: Users },
  { to: "/m/sales", label: "销售", icon: ShoppingCart },
  { to: "/m/reconciliation", label: "对账", icon: Scale },
];

const drawerItems = [
  { group: "客户", items: [
    { to: "/m/customers", label: "客户档案", icon: Users },
    { to: "/m/contacts", label: "联系人", icon: UserSquare2 },
    { to: "/m/follow-ups", label: "跟进记录", icon: MessageSquareText },
  ]},
  { group: "供应链", items: [
    { to: "/m/products", label: "产品", icon: Package },
    { to: "/m/suppliers", label: "供应商", icon: Truck },
    { to: "/m/purchases", label: "采购订单", icon: FileBox },
  ]},
  { group: "销售与财务", items: [
    { to: "/m/sales", label: "销售订单", icon: ShoppingCart },
    { to: "/m/payments", label: "账务收支", icon: Wallet },
...
function Drawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed top-0 right-0 bottom-0 w-[78%] max-w-[320px] bg-card z-50 shadow-2xl transition-transform overflow-y-auto",
            ...
            <div key={g.group} className="mb-3">
              ...
              {g.items.map((it) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-foreground/5 text-left"
                >
...
export default function MobileLayout() {
  ...
  // hide bottom nav on detail/edit pages (deeper than /m/<section>)
  const segs = loc.pathname.replace(/^\/m\/?/, "").split("/").filter(Boolean);
  const hideBottom = segs.length >= 2; // e.g. /m/customers/abc
      ...
      <header className="h-12 flex items-center justify-between px-4 border-b border-foreground/10 bg-card/80 backdrop-blur sticky top-0 z-30">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => setDrawer(true)} className="size-9 rounded-full hover:bg-foreground/5 flex items-center justify-center">
            <MenuIcon className="h-4.5 w-4.5 text-foreground/75" />
          </button>
        </div>
      </header>

      <main className={cn("flex-1 overflow-y-auto", hideBottom ? "pb-4" : "pb-20")}>
        <Outlet />
      </main>
