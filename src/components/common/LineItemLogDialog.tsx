                  ...
                  <div key={l.id} className="rounded-lg border border-border bg-card p-3 text-xs">
                    ...
                    {(l.action === "add" || l.action === "delete") && l.snapshot && (
                      <div className="grid grid-cols-4 gap-2 text-[12px] pl-1 text-foreground/75">
                        <div>分类：<span className="font-semibold text-foreground">{l.snapshot.category && l.snapshot.category in productCategoryLabel ? productCategoryLabel[l.snapshot.category as keyof typeof productCategoryLabel] : l.snapshot.category}</span></div>
